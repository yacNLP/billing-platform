...
export function InvoicesList({ action }: InvoicesListProps) {
  ...
  const selectedCustomer = queryParams.customerId
    ...
    : undefined;
  const selectedSubscription = queryParams.subscriptionId
    ? subscriptions?.data.find(
        (subscription) => subscription.id === queryParams.subscriptionId,
      )
    : undefined;
  const selectedSubscriptionCustomer = selectedSubscription
    ? customers?.data.find(
        (customer) => customer.id === selectedSubscription.customerId,
      )
    : undefined;
    ...
    },
    {
      ...
      value: selectedSubscription
        ? `Subscription #${selectedSubscription.id}${
            selectedSubscriptionCustomer
              ? ` · ${selectedSubscriptionCustomer.name}`
              : ""
          }`
        : queryParams.subscriptionId
          ? `Subscription ID ${queryParams.subscriptionId}`
          : "All subscriptions",
    },
    {
      label: "Page size",
            ...
            >
              ...
              <div className="flex flex-wrap items-center gap-2.5 md:col-span-2 xl:col-span-4">
                ...
                <p className="text-sm text-slate-500">
                  {data.total} invoices found. Page {data.page} of{" "}
                  {data.totalPages || 1}.
                </p>
              </div>
            </form>
